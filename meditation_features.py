...
def run_meditation_session(duration_minutes):   
    ...
    # ✅ Only show header, button, etc. when session is actually running
    ...
    # Ensure resources are released properly
        ...
        counts = voice_manager.feedback_counts
        st.markdown("### ✅ Meditation session ended.")
        st.markdown("## 🧾 Session Feedback Summary")
        
        issues, tips, positives = [], [], []

        def severity(count): return "⚠" if count >= 3 else "🔸"

        if counts["eyes"]:
            issues.append(f"{severity(counts['eyes'])} You opened your eyes multiple times.")
            tips.append("- Try to keep your eyes closed throughout the meditation.")
        else:
            positives.append("- ✅ Great job keeping your eyes closed.")

        if counts["posture"]:
            issues.append(f"{severity(counts['posture'])} Your posture was incorrect at times.")
            tips.append("- Focus on maintaining a straight and relaxed posture.")
        else:
            positives.append("- ✅ Excellent posture control!")

        if counts["head"]:
            issues.append(f"{severity(counts['head'])} Your head was tilted during the session.")
            tips.append("- Try to keep your head aligned with your shoulders.")
        else:
            positives.append("- ✅ Head alignment was good.")

        if counts["shoulders"]:
            issues.append(f"{severity(counts['shoulders'])} Your shoulders were not level.")
            tips.append("- Keep your shoulders level and relaxed.")
        else:
            positives.append("- ✅ Shoulder balance was maintained.")

        if counts["breathing_none"]:
            issues.append(f"{severity(counts['breathing_none'])} At times, you weren't visibly breathing.")
            tips.append("- Ensure you're breathing gently and consistently.")
        if counts["breathing_harsh"]:
            issues.append(f"{severity(counts['breathing_harsh'])} Your breathing was harsh.")
            tips.append("- Try to breathe calmly and evenly.")
        if not counts["breathing_none"] and not counts["breathing_harsh"]:
            positives.append("- ✅ Breathing was calm and consistent.")
